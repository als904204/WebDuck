...
public class FakeReviewRepository implements ReviewRepository {
    ...
    private final List<Review> reviewsData = Collections.synchronizedList(new ArrayList<>());
    private final List<ReviewLikes> reviewLikesData = Collections.synchronizedList(new ArrayList<>());
    ...
    public Review save(Review review) {
        if (review.getId() == null || review.getId() == 0) {
            ...
            reviewsData.removeIf(item -> Objects.equals(item.getId(), review.getId()));
    ...
    public Optional<Review> findById(Long id) {
        return reviewsData.stream().filter(
    ...
    public void deleteById(Long id) {
        reviewsData.removeIf(review -> review.getId().equals(id));
    }

    @Override
    public int countReviewsByWebtoonId(Long webtoonId) {
        return (int) reviewsData.stream()
    ...
    public boolean existsById(Long id) {
        return reviewsData.stream().anyMatch(review -> review.getId().equals(id));
    }
    ...
    public void deleteReviewLikesById(Long id) {
        reviewLikesData.removeIf(item -> item.getId().equals(id));
    }
    ...
    public ReviewLikes saveReviewLikes(ReviewLikes reviewLikes) {
        if (reviewLikes.getId() == null || reviewLikes.getId() == 0) {
            ...
            reviewLikesData.add(newReviewLikes);
            return newReviewLikes;
        }
        reviewLikesData.removeIf(item -> Objects.equals(item.getId(), reviewLikes.getId()));
        reviewLikesData.add(reviewLikes);
        return reviewLikes;
    }

    @Override
    public Optional<ReviewLikes> findReviewLikesByReviewIdAndMemberId(Long reviewId, Long memberId){
        return reviewLikesData.stream()
