  ...
  <div class="p-4 grid justify-content-center">
    <div class="col-12 md:col-6 lg:col-4">
      <div class="flex flex-column gap-2">
        ...
        <Button label="등록" class="mt-1" @click="updateMyCollection" />
      </div>
    </div>
  </div>
</template>
...
import {
  fetchMyCollectionById,
  updateCollection
} from "../../service/CollectionService.js";
...
onMounted(async (id) => {
  try {
    const response = await fetchMyCollectionById(collectionId);
    myCollection.value = response;

    title.value = response.title;
    description.value = response.description;
    selectedWebtoonIds.value = response.webtoonInfo.map(info => info.id);
    selectedWebtoonTitle.value = response.webtoonInfo.map(info => info.title);
...
const updateMyCollection = async () => {
